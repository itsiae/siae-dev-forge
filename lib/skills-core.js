...
function buildCatalog(pluginDir) {
  ...
  const disambiguation = [
    ...
    '- Query su C4, HLD, bounded context, CQRS, microservizi vs monolite, pattern architetturale → `siae-architecture` (NON brainstorming)',
    '- Query su Playwright, Cypress, test E2E, CI/CD pipeline, GitHub Actions, automatizza test → `siae-automation` (NON brainstorming)',
    '- Query su Glue, PySpark, ETL, pipeline ingestion, Medallion, bronze-to-silver → `siae-data-engineering` (NON brainstorming)',
    '- Query su Terraform, terragrunt, VPC, ECS, Lambda, security group → `siae-iac` (NON brainstorming)',
    '- Query su bug, errore, stacktrace, crash, fallisce, non funziona → `siae-debugging` (NON brainstorming)',
    '- Query su git checkout, git commit, git push, branch, merge, tag → `siae-git-workflow` (NON brainstorming)',
    '- `siae-brainstorming` SOLO quando l\'utente vuole progettare una feature nuova generica senza keyword specifiche sopra',
  ];
