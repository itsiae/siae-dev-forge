...
function extractFrontmatter(filePath) {
  ...
  // Extract backbone_role
  const roleMatch = yaml.match(/^backbone_role:\s*(.+)$/m);
  if (roleMatch) {
    result.backbone_role = roleMatch[1].trim();
  }

  // Extract backbone_stage
  const stageMatch = yaml.match(/^backbone_stage:\s*(.+)$/m);
  if (stageMatch) {
    const stageVal = stageMatch[1].trim();
    result.backbone_stage = (stageVal === 'null' || stageVal === '') ? null : stageVal;
  }

  // Extract hard_gate
  const gateMatch = yaml.match(/^hard_gate:\s*(.+)$/m);
  if (gateMatch) {
    const gateVal = gateMatch[1].trim();
    result.hard_gate = gateVal === 'true';
  }
...
function buildCatalog(pluginDir) {
  ...
  const disambiguation = [
    '',
    '**Regola di priorita\' — failure intent beats domain intent:**',
    '- Se la query contiene sintomi di failure (errore, crash, timeout, non funziona, fallisce, down, lento, regressione), `siae-debugging` VINCE SEMPRE come PRIMA skill. La skill di dominio arriva DOPO.',
    '- Glue/Lambda + failure → `siae-debugging` prima, poi `siae-data-engineering`/`siae-iac`',
    '- Frontend + CORS/500/loop → `siae-debugging` prima, poi `siae-frontend`',
    '- Terraform + fallisce → `siae-debugging` prima, poi `siae-iac`',
    '- Test fallisce + qualsiasi dominio → `siae-debugging` prima',
    '- Discrepanza dati, numeri che non tornano, record mancanti/duplicati, output sbagliato, riconciliazione, stato impossibile, null inatteso → `siae-debugging` prima. Se c\'e\' contesto ETL/Glue/PySpark → poi `siae-data-engineering`',
    '',
    '**Disambiguazione skill (quando piu\' skill matchano, SENZA failure):**',
    ...
    '- `siae-brainstorming` SEMPRE per QUALSIASI task implementativo (feature, bug fix dopo RCA, refactoring, config, ottimizzazione — zero eccezioni).',
  ];
...
if (require.main === module) {
  ...
  const format = (process.argv[3] || 'markdown').toLowerCase();
  const catalog = buildCatalog(pluginDir);

  if (format === 'json') {
    const jsonSkills = catalog.skills.map(s => ({
      name: s.name,
      triggers: s.trigger ? s.trigger.split(/,\s*/) : [],
      type: s.type,
      phase: s.phase,
    }));
    process.stdout.write(JSON.stringify(jsonSkills, null, 2) + '\n');
  } else {
