...
**Nota:** T09 è già DONE — `Write-InstallLog` e `Invoke-DevForgeCommand` sono disponibili in `install.ps1` (righe 26-90). Il test mocka direttamente `Invoke-DevForgeCommand` per evitare chiamate reali a `winget`/`choco` e `Write-InstallLog` scrive su log file senza impatto funzionale. ParameterFilter usa `$Executable` + `$Arguments -contains '...'` (signature splat, no string matching).
