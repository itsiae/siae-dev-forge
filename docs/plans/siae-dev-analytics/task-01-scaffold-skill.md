# Task 01 — Skill Scaffold + SKILL.md + Gate Privacy

**Goal:** Creare la struttura directory della skill `siae-dev-analytics` con SKILL.md, trigger keyword, card 🔴 ALTO privacy, requirements.txt, template config.

**AC coperti:** AC01, AC02, AC11

**Dipendenze:** nessuna (primo task)

**Tempo stimato:** 15 min

---

## File coinvolti

- `skills/siae-dev-analytics/SKILL.md` (nuovo)
- `skills/siae-dev-analytics/scripts/requirements.txt` (nuovo)
- `skills/siae-dev-analytics/template/devforge-analytics.yml` (nuovo)
- `skills/siae-dev-analytics/reference/` (directory vuota, placeholder per Task 7)
- `skills/siae-dev-analytics/tests/__init__.py` (nuovo, vuoto)
- `skills/siae-dev-analytics/tests/fixtures/` (directory)

## Step 1 — Crea struttura directory

Run:
```bash
cd "/Users/detomasi/Library/Mobile Documents/com~apple~CloudDocs/siae-dev-forge"
mkdir -p skills/siae-dev-analytics/{scripts,reference,template,tests/fixtures}
touch skills/siae-dev-analytics/tests/__init__.py
```

Output atteso: 6 directory create, 1 file vuoto.

## Step 2 — Scrivi `skills/siae-dev-analytics/SKILL.md`

Crea file con questo contenuto esatto:

````markdown
---
name: siae-dev-analytics
description: Misura velocità e qualità degli sviluppatori SIAE che usano Claude Code + DevForge. Genera report Excel con 11 KPI (DORA + DX AI Measurement) + ROI Index. Trigger keywords, "misura produttività dev", "ROI Claude Code", "KPI sviluppatori", "analytics dev", "report performance team", "/forge-analytics", "dev metrics", "velocity quality report", "dashboard produttività", "cosa fanno gli sviluppatori", "benchmark dev", "ROI AI coding".
---

# siae-dev-analytics — Developer Analytics & ROI Report

> **Tipo:** Rigid | **Fase SDLC:** 5. Review/Metrics

## LA LEGGE DI FERRO

```
DATI NOMINATIVI SOLO DOPO CONFERMA ESPLICITA — CARD 🔴 ALTO OBBLIGATORIA
```

## Scopo

Analizza repository GitHub SIAE per calcolare KPI velocity + quality per sviluppatore, produce report Excel consumabile da management.

**AUTO-BEST mode:** la skill rileva automaticamente le fonti dati disponibili e tira il massimo:
- GitHub (sempre obbligatorio, ground truth)
- S3 telemetria DevForge (opzionale, se disponibile blend con costi Claude Code)
- Git trailers locali (se repo clonato)

## Flow

### Step 1 — Check ambiente

Run:
```bash
gh auth status 2>&1 | grep -q "Logged in" || echo "ABORT: gh not authenticated"
python3 --version | awk '{print $2}' | awk -F. '{if ($1 < 3 || $2 < 10) print "ABORT"}'
pip install -r skills/siae-dev-analytics/scripts/requirements.txt 2>&1 | tail -3
```

Se ABORT → mostra errore e ferma.

### Step 2 — Carica config

1. Cerca `devforge-analytics.yml` nella working directory
2. Se non esiste → prompt utente per scope minimo (repos + finestra)
3. Valida con Pydantic

### Step 3 — Autodetect sources

Run:
```bash
python3 skills/siae-dev-analytics/scripts/run_analytics.py autodetect --config <path>
```

Output atteso (JSON):
```json
{"github": true, "s3_devforge": false, "s3_blend": false, "mode": "GITHUB-ONLY"}
```

### Step 4 — Gate 🔴 ALTO privacy

Mostra card con N dev, N repo, finestra, mode. Richiedi conferma esplicita.

**Template card:**

| 🔴 ALTO (dati personali sviluppatori) — 🔨 DevForge · siae-dev-analytics |
|:---|
| **⚠️ OPERAZIONE CON DATI PERSONALI** |
| 👥 Dev coinvolti: `<N>` · 📅 Finestra: `<from → to>` · 📁 Repo: `<N>` · 🎯 Mode: `<FULL/HYBRID/GITHUB-ONLY>` |
| **▼ Azione** |
| 1. 📊 Fetch dati GitHub nominativi + calcolo KPI individuali |
| 2. 📤 Output Excel con tabelle per-dev |
| 💡 Perche': Valutazione ROI Claude Code + reportistica management |
| 🚫 Se NO: Abort. Nessun fetch, nessun file prodotto. |
| 🔒 Alternativa: rerun con `--anonymize` → hash SHA256 dei login |

Solo dopo conferma esplicita utente → procedi.

### Step 5 — Pipeline

Run:
```bash
python3 skills/siae-dev-analytics/scripts/run_analytics.py run --config <path> [--anonymize] [--format xlsx|csv|both]
```

### Step 6 — Report narrativo

Dopo produzione Excel, Claude genera markdown summary con:
- Top 3 dev per ROI Index
- Insight principali (es. "il team ha cycle time mediano 24h, in linea con DORA elite")
- Raccomandazioni management
- Path file Excel prodotto

## Trigger Keywords

- "misura produttività dev"
- "ROI Claude Code"
- "KPI sviluppatori"
- "analytics dev"
- "report performance team"
- "/forge-analytics"
- "dev metrics"
- "velocity quality report"
- "dashboard produttività"
- "cosa fanno gli sviluppatori"
- "benchmark dev"
- "ROI AI coding"

## Limiti Operativi

| Vincolo | Limite | Se superato |
|---------|--------|-------------|
| Repo max per run | 50 | Batch in più run |
| Dev max per run | 100 | Report diventa rumoroso |
| Finestra max | 365gg | Spezza in più run annuali |

## Anti-Pattern

| Pensiero | Realtà |
|----------|--------|
| "È solo un report, non serve card 🔴" | Dati nominativi = GDPR. Card sempre. |
| "LOC/giorno è una metrica valida" | Anti-pattern DORA. Non includerla mai. |
| "Self-reporting dev sarebbe utile" | Perception gap documentato (METR 2025). Solo metriche oggettive. |
| "Posso saltare autodetect, so cosa c'è" | L'ambiente cambia. Autodetect sempre. |

## Permission Denied Handling

- Se `pip install` negato → prompt card 🟡 MEDIO con alternative (venv, pipx, uv)
- Se `gh` non autenticato → ABORT con istruzioni `gh auth login`
- Se S3 creds mancanti → graceful degrade GITHUB-ONLY, noted in report
````

## Step 3 — Scrivi `scripts/requirements.txt`

Contenuto esatto:

```
pandas>=2.0
openpyxl>=3.1
pydantic>=2.0
pyyaml>=6.0
boto3>=1.28
pytest>=7.4
pytest-cov>=4.1
```

## Step 4 — Scrivi `template/devforge-analytics.yml`

Contenuto esatto:

```yaml
# DevForge Analytics — Config Template
# Copia questo file nel tuo progetto e personalizzalo.
# Path finale: <progetto>/devforge-analytics.yml

version: 1

scope:
  repos:
    - itsiae/catalogo-service
    - itsiae/diritti-service
  teams: []                           # es. "itsiae/team-backend"
  topics: []                          # es. "siae-microservice"

time_window:
  from: "2026-01-01"                  # ISO date, REQUIRED
  to: "today"                         # "today" o ISO date

developers:
  include: []                         # vuoto = tutti
  exclude:
    - "dependabot[bot]"
    - "renovate[bot]"
    - "github-actions[bot]"

options:
  anonymize: false                    # true = hash SHA256(login)[:8]
  min_commits_threshold: 5
  parallel_fetch: 4

output:
  format: xlsx                        # xlsx | csv | both
  path: ./devforge-analytics-report.xlsx
```

## Step 5 — Crea `tests/conftest.py` per pytest

Contenuto esatto:

```python
"""Pytest fixtures condivisi per siae-dev-analytics."""
import json
import pytest
from pathlib import Path


@pytest.fixture
def fixtures_dir() -> Path:
    """Directory fixtures con dati mock deterministici."""
    return Path(__file__).parent / "fixtures"


@pytest.fixture
def sample_pr_data(fixtures_dir: Path) -> dict:
    """Sample GitHub PR data (5 PR, 3 dev)."""
    fixture = fixtures_dir / "github_api_response.json"
    if not fixture.exists():
        pytest.skip("fixture missing, run task-03 first")
    return json.loads(fixture.read_text())


@pytest.fixture
def sample_commit_data(fixtures_dir: Path) -> dict:
    """Sample commit data (50 commit cross-dev)."""
    fixture = fixtures_dir / "commits_sample.json"
    if not fixture.exists():
        pytest.skip("fixture missing, run task-03 first")
    return json.loads(fixture.read_text())
```

## Step 6 — Verifica struttura

Run:
```bash
cd "/Users/detomasi/Library/Mobile Documents/com~apple~CloudDocs/siae-dev-forge"
find skills/siae-dev-analytics -type f | sort
```

Output atteso:
```
skills/siae-dev-analytics/SKILL.md
skills/siae-dev-analytics/scripts/requirements.txt
skills/siae-dev-analytics/template/devforge-analytics.yml
skills/siae-dev-analytics/tests/__init__.py
skills/siae-dev-analytics/tests/conftest.py
```

## Step 7 — Commit

Run:
```bash
cd "/Users/detomasi/Library/Mobile Documents/com~apple~CloudDocs/siae-dev-forge"
git add skills/siae-dev-analytics/
git commit -m "feat(skill): scaffold siae-dev-analytics skill [Task 1/7]

- SKILL.md con 12 trigger keyword, card 🔴 privacy, flow 6 step
- requirements.txt (pandas, openpyxl, pydantic, pyyaml, boto3, pytest)
- Template devforge-analytics.yml con scope/window/options
- tests/ con conftest.py fixtures condivisi

AC01, AC02, AC11"
```

Output atteso: commit success, 5 file creati.

## Criteri di accettazione Task 1

- [ ] Directory `skills/siae-dev-analytics/` esiste con subdirectory corrette
- [ ] `SKILL.md` contiene frontmatter, 12 trigger, flow 6 step, card 🔴 template
- [ ] `requirements.txt` lista le 7 dipendenze
- [ ] `template/devforge-analytics.yml` è YAML valido (`python -c 'import yaml; yaml.safe_load(open("skills/siae-dev-analytics/template/devforge-analytics.yml"))'`)
- [ ] `conftest.py` con fixtures condivisi
- [ ] Commit creato con messaggio conventional

## Verifica

Run:
```bash
python3 -c "import yaml; print(yaml.safe_load(open('skills/siae-dev-analytics/template/devforge-analytics.yml')))"
```

Output atteso: dict stampato con chiavi `version`, `scope`, `time_window`, `developers`, `options`, `output`.

Run:
```bash
grep -c "^" skills/siae-dev-analytics/SKILL.md
```

Output atteso: ≥ 80 righe.
