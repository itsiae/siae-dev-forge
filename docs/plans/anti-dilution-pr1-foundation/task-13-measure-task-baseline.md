# Task 13 — Implementare lib/measure-task-baseline.sh

**Stato:** [PENDING]
**Execution:** in-session
**Dipendenze:** nessuna (usa solo snapshot già esistente)
**Durata stimata:** 15 min

## Goal

Generare `baseline-metrics-tasks.json` simulando task-scope sul baseline session-scoped. Serve come **pre-measurement** per misurare lift reale post-PR #2.

Il task_id reale richiede PR #2 (ADR-001). Per il baseline PR #1, usiamo un proxy:

```
proxy_task_id(session) = hash(branch_of_session + design_doc_path_if_present)
```

Questo approssima il task_id futuro senza richiedere l'implementazione PR #2.

## Step

### Step 1 — Read schema dati

```bash
# Un evento con branch
head -1 $(find docs/measurements/baseline-2026-04-25/devforge-state-snapshot -name activity.jsonl | head -1) | python3 -m json.tool | head -20
```

Verificato: ogni evento ha `branch` nella radice.

### Step 2 — Crea lib/measure-task-baseline.sh

File: `lib/measure-task-baseline.sh`
...
```bash
chmod +x lib/measure-task-baseline.sh
bash lib/measure-task-baseline.sh
```
Output atteso: `Wrote docs/measurements/baseline-2026-04-25/baseline-metrics-tasks.json`.
...
python3 -c "
import json
d = json.load(open('docs/measurements/baseline-2026-04-25/baseline-metrics-tasks.json'))
print('tasks_identified:', d['totals']['tasks_identified'])
print('tasks_with_commit:', d['totals']['tasks_with_commit'])
print('session/task ratio:', d['totals']['session_to_task_ratio'])
print('brainstorming per-task:', d['adoption_per_task_pct']['with_commit']['siae-brainstorming'], '%')
print('tdd per-task:', d['adoption_per_task_pct']['with_commit']['siae-tdd'], '%')
print('verification per-task:', d['adoption_per_task_pct']['with_commit']['siae-verification'], '%')
"
```
Output atteso: numeri coerenti (tasks_identified > sessions, session/task ratio <= 1 tipicamente ~1 per proxy semplice).

### Step 5 — Commit

```bash
git add lib/measure-task-baseline.sh docs/measurements/baseline-2026-04-25/baseline-metrics-tasks.json
git commit -m "feat(lib): add measure-task-baseline.sh + proxy task-scoped baseline

Part of PR #1 anti-dilution (pre-measurement for PR #2 lift).
Generates baseline-metrics-tasks.json via proxy: task_id = sha256(branch + design_doc).
Caveat documented: real task_id from PR #2 may differ.
Serves as order-of-magnitude reference to detect real lift post-PR #2 migration."
```

## Acceptance

- [ ] `lib/measure-task-baseline.sh` creato e executable
- [ ] `baseline-metrics-tasks.json` generato nel baseline dir
- [ ] JSON contains: tasks_identified, adoption_per_task_pct, tasks_sample
- [ ] Caveat documentato nel JSON
- [ ] Commit `feat(lib):`
