# Task 03 — Test osservabilità identità (RED)

**Tipo:** test (TDD RED) · **AC:** AC4, AC5, AC8 · **File:** `tests/hooks/test_identity_observability.sh` (nuovo)

## Obiettivo

Testare la logica di emissione `identity_external_domain` / `identity_unresolved`.

## Decisione di testabilità

`session-start` è un hook pesante (rete, NTP) → run completo flaky in sandbox
(memory *session-start test env flaky*). Quindi l'emit viene estratto in una funzione
unit-testabile di `lib/logger.sh`: `devforge_emit_identity_observability`, chiamata dal
branch `startup)`. AC4/AC5 testano la funzione in isolamento; AC8 (no emit fuori da startup)
è verifica **strutturale** sul call-site.

## Implementazione

Crea `tests/hooks/test_identity_observability.sh`:
...
`chmod +x tests/hooks/test_identity_observability.sh`.
...
**Atteso:** FAIL (exit 1) — `devforge_emit_identity_observability` non esiste e la chiamata non è in session-start.

## Done quando

- File creato ed eseguibile.
- Eseguito ora: exit≠0 (RED confermato).
