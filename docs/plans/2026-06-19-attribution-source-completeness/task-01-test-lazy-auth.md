# Task 01 — Test lazy auth resolution (RED)

**Tipo:** test (TDD RED) · **AC:** AC1, AC2, AC3, AC6, AC7 · **File:** `tests/hooks/test_lazy_auth_resolution.sh` (nuovo)

## Obiettivo

Scrivere il test che **fallisce** prima dell'implementazione di `_devforge_ensure_auth`:
verifica che `auth_email` venga risolto da `~/.claude.json` quando `DEVFORGE_AUTH_EMAIL` NON è pinnato.

## Contesto

- Idiom di riferimento: `tests/hooks/test_log_toplevel_attribution.sh` (source di `lib/logger.sh`, `assert_json_field` via python3, repo tmp con `git init`).
- Resolution legge `DEVFORGE_CLAUDE_JSON` (override di `~/.claude.json`) — vedi `lib/logger.sh:395` `devforge_resolve_auth_identity`.

## Implementazione

Crea `tests/hooks/test_lazy_auth_resolution.sh`:
...
`chmod +x tests/hooks/test_lazy_auth_resolution.sh`.
...
**Atteso:** FAIL (exit 1) su AC1/AC2 perché `_devforge_ensure_auth` non esiste ancora → `auth_email` vuoto.
Conferma il fallimento via **exit code** (non grep PASS — memory *Test verify via exit code*).

## Done quando

- File creato ed eseguibile.
- Eseguito ora: fallisce con exit≠0 sui casi lazy (RED confermato).
