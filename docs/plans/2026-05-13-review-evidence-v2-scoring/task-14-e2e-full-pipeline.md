# Task 14 — E2E test full pipeline v2

**SP:** 1.5 · **AC mappati:** AC #8 (coverage), AC #14 · **Dipendenze:** Task 09-13 · **Wave:** 4

## Goal

Test E2E: hook bash → collector orchestrate_v2 → runners + scoring + baseline_cache (S3 mock) → evidence v2 JSON → reviewer agent contract. Verifica che la pipeline completa funziona dal trigger fino al verdict finale.

## File coinvolti

**Creare:**
- `tests/test_review_evidence_e2e_v2.py`
...
```bash
python3 -m pytest tests/test_review_evidence_e2e_v2.py -v
# 6 passed atteso

# Full suite gate
python3 -m pytest tests/ --cov=lib.review_evidence -q

git add tests/test_review_evidence_e2e_v2.py
git commit -m "test(review-evidence-v2): E2E full pipeline + coverage gate (#task-14)"
```

## Criteri di accettazione

- [ ] E2E first PR baseline_synthetic → AUTO_APPROVE
- [ ] E2E BLOCK_HARD_FLOOR scenario → decision:block + BREAK-GLASS info
- [ ] E2E SEVERELY_DEGRADED scenario → no block, advisory
- [ ] E2E v1 evidence backward-compat (no crash)
- [ ] **AC #14:** Coverage `lib/review_evidence/` ≥85%
- [ ] Full suite passes (v1 + v2)
- [ ] 6 E2E test PASS
