...
Implementare `lib/release_risk/renderer.py` che fill template `release-criticality-checklist.md` con `ReleaseRiskReport`. Output con 4 livelli LOW/MED/HIGH/CRIT, visual coding ✅/❌/⚠️, SUGGESTED FOLLOW-UP block, warning su `partial=True`.

## File coinvolti

- Create: `lib/release_risk/renderer.py`

## Step

### Step 1 — Write renderer.py

Write `lib/release_risk/renderer.py`:
```python
...
```bash
python3 -c "from lib.release_risk.renderer import render_scorecard, write_scorecard; print('OK')"
```

### Step 3 — Commit

```bash
git add lib/release_risk/renderer.py
git commit -m "feat(release-risk): renderer markdown scorecard (4 livelli + followup + partial warning)"
```

## Criteri di accettazione

- [ ] Visual coding ✅/❌ inverted per negative-weight criteria
- [ ] SUGGESTED FOLLOW-UP block visibile se followups presenti
- [ ] Warning su partial=True
- [ ] Genesis section 3-outcome
- [ ] Atomic write
- [ ] Commit eseguito
