...
Test 5 scenari genesis: no merges, all confirmed, partial confirmed (anomaly), declined, parser regex edge cases.

## File coinvolti

- Create: `tests/test_release_risk_genesis.py`

## Step

### Step 1 — Scrivi test

Write `tests/test_release_risk_genesis.py`:
```python
...
Run:
```bash
pytest tests/test_release_risk_genesis.py -v
```
...
```bash
git add tests/test_release_risk_genesis.py
git commit -m "test(release-risk): genesis 5 scenarios + parser regex 3 format"
```

## Criteri di accettazione

- [ ] 9 test PASSED
- [ ] Test 4 outcome (no merges, declined, all-confirmed, anomaly)
- [ ] Test parser regex 3 formati merge subject
- [ ] Test subprocess error fallback (empty list)
- [ ] Commit eseguito
