# Task 05 — [TDD] test_release_risk_schema.py
...
Unit test per `lib/release_risk/schema.py`: serialization roundtrip + Literal type validation.

## File coinvolti

- Create: `tests/test_release_risk_schema.py`

## Step

### Step 1 — Scrivi test

Write `tests/test_release_risk_schema.py`:
```python
...
### Step 2 — Verifica fail prima (red)

Run:
```bash
pytest tests/test_release_risk_schema.py -v
```
Output atteso: 6 PASSED (perché schema.py già implementato in task-04).

### Step 3 — Commit

Run:
```bash
git add tests/test_release_risk_schema.py
git commit -m "test(release-risk): schema dataclass roundtrip + Literal type validation"
```

## Criteri di accettazione

- [ ] 6 test PASSED
- [ ] Roundtrip JSON verifica preservation dataclass nesting
- [ ] Tutti Literal type validi
- [ ] Commit eseguito
