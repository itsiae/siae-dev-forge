# Task 03 — Scrivi `run.sh` (Bedrock Runner)

**Goal:** Bash runner che per ogni case in `cases.yml` invoca Bedrock con il prompt + contesto SKILL.md descrizioni, capture output, scrive log JSONL per evaluator.py.

**File coinvolti:**
- `tests/skill-activation/run.sh` (nuovo, eseguibile)

## Step 1 — Scrivi runner

In `tests/skill-activation/run.sh`:
...
## Step 2 — Make eseguibile

```bash
chmod +x tests/skill-activation/run.sh
```

## Step 3 — Verifica syntax bash

```bash
bash -n tests/skill-activation/run.sh && echo "syntax OK"
```

## Step 4 — Commit

```bash
git add tests/skill-activation/run.sh
git commit -m "test(skill-activation): runner Bedrock Sonnet/Haiku per cases.yml

Iter 30 case (3 in --smoke mode), invoca Bedrock InvokeModel con system prompt
contenente description di tutte skill DevForge. Output JSONL in .cache/."
```

## Criteri accettazione

- `run.sh` eseguibile (chmod +x)
- `bash -n run.sh` passa syntax check
- Supporta `--smoke` flag
- Supporta `TEST_MODEL=sonnet|haiku`
- Errore esplicito se `AWS_BEARER_TOKEN_BEDROCK` non set

## Note no-regression

Il runner NON modifica nessuna skill. Pure read-only su skills/. Questo task è infrastrutturale, no-regression non applicabile.
