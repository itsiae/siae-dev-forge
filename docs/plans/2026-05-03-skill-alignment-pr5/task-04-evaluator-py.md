# Task 04 — Scrivi `evaluator.py`

**Goal:** Python script che parsa log JSONL del runner, confronta con `cases.yml` expected, calcola KPI, scrive report markdown.

**File coinvolti:**
- `tests/skill-activation/evaluator.py` (nuovo)

## Step 1 — Scrivi evaluator

In `tests/skill-activation/evaluator.py`:
...
```bash
python3 -c "import py_compile; py_compile.compile('tests/skill-activation/evaluator.py', doraise=True)"
```

## Step 3 — Test con dummy log

Crea log finto e testa:

```bash
cat > /tmp/dummy.jsonl <<'EOF'
{"id": "bug-npe-endpoint", "prompt": "ho un NPE", "response": "{\"primary\": \"siae-debugging\", \"chain\": [\"siae-debugging\", \"siae-tdd\"]}"}
EOF
cd tests/skill-activation && python3 evaluator.py /tmp/dummy.jsonl --label dummy
ls report-*-dummy.md
```

Output atteso: report markdown generato, accuracy 100% per il singolo case.

## Step 4 — Cleanup

```bash
rm -f tests/skill-activation/report-*-dummy.md /tmp/dummy.jsonl
```

## Step 5 — Commit

```bash
git add tests/skill-activation/evaluator.py
git commit -m "test(skill-activation): evaluator Python per parsing log + report markdown KPI"
```

## Criteri accettazione

- `evaluator.py` syntax-valid
- Smoke test con dummy log produce report markdown
- KPI calcolati: activation_accuracy, forbidden_rate, chain_completeness

## Note no-regression

Pure infrastrutturale, nessuna skill modificata.
