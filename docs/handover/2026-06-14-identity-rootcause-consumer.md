...
| `duration_source` | eventi temporizzati (`devforge_log_timed`, es. `session_start`, `session_end`, `commit_created`, `pr_opened`, `pr_merged`) | Valore fisso `"wallclock"`: indica che `duration_ms` è misurato via epoch-ns (clock di parete), non tempo CPU. Il consumer deve applicare cap e mediana — il producer non li applica (ADR-4). **IMPORTANTE:** gli eventi emessi via `devforge_log` (non-timed, es. `task_adoption`, `gate_bypassed`, skill hooks senza misurazione) NON portano il campo `duration_source` — la chiave è **ASSENTE**, non null. Il consumer deve filtrare per tipo evento (`session_start`/`session_end`/`commit_created`/`pr_opened`/`pr_merged`) per identificare gli eventi con duration, **non** usare `duration_source IS NOT NULL` (filtra solo i non-timed ma non è robusto a estensioni future). | `lib/logger.sh` (`devforge_log_timed`, linea 775) |
